...
public class Servidor {
    ...
    private static final String[] RESPUESTAS = { "paris", "marte", "agua", "amazonas", "luna" };
    ...
    private static void registrarRespuesta(String respuesta, String ip) {
        try (PrintWriter writer = new PrintWriter(new FileWriter("respuestasCLiente.txt", true))) {
            String fechaHora = java.time.LocalDateTime.now().format(java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
            String entrada = String.format("Pregunta #%d\nFecha y Hora: %s\nIP: %s\nRespuesta: %s\n", 
